...
async def vi(ctx,m:discord.Member=None):
    ...
    vip="👑 **VIP**" if any(r.name.lower()=="vip" for r in m.roles) else "Người chơi Thường"
...
async def muarole(ctx,role=None):
    ...
    u=user(ctx.author.id,ctx.author.name)
    if u["cash"]<30000000:
        return await ctx.send("❌ Không đủ tiền! VIP giá **30,000,000$**.")

    r=discord.utils.find(lambda x:x.name.lower()=="vip",ctx.guild.roles)
    if not r:return await ctx.send("❌ Server chưa có role **Vip**.")

    if r in ctx.author.roles:
        return await ctx.send("👑 Bạn đã là VIP!")

    if r >= ctx.guild.me.top_role:
        return await ctx.send("❌ Hãy kéo role **Vip** xuống dưới role của Bot.")
    ...
    await ctx.send(embed=E(
        "👑 MUA VIP THÀNH CÔNG",
        f"🎉 {ctx.author.mention} đã trở thành **VIP**!\n\n"
        "💰 Giá: `30,000,000$`\n"
        "⚡ Thưởng: **x1.5 tiền thắng**\n"
        "🍀 May mắn: **+1%**\n\n"
        f"💵 Ví: `{u['cash']:,}$`",GREEN))

def vip(ctx):
    return any(r.name.lower()=="vip" for r in ctx.author.roles)

def payout(ctx,bet,mult=2):
    return int(bet*mult*(1.5 if vip(ctx) else 1))
...
async def tx(ctx,ch=None,bet:int=None):
    ...
    if not TX["on"]:
        ...
        TX["msg"]=await ctx.send(embed=E(
            ...
            "Gõ `!tx <tai/xiu> <tiền>`\n\n"
            "⏱️ **Thời gian: 30 giây**\n\n"
            f"💰 **Tổng Tài:** `{bet if ch=='tai' else 0:,}$`\n"
            f"💰 **Tổng Xỉu:** `{bet if ch=='xiu' else 0:,}$`",
            ORANGE))
...
async def txround():
    ...
    for i,b in TX["bets"].items():
        if b["choice"]==r:
            money=payout(
                type("X",(),{"author":type("A",(),{"roles":[]})()})(),
                b["amount"],2
            )
            # VIP được xử lý riêng bằng dữ liệu role
            member=None
            for g in bot.guilds:
                member=g.get_member(i)
                if member:break
            money=int(b["amount"]*2*(1.5 if member and any(
                z.name.lower()=="vip" for z in member.roles) else 1))
            user(i)["cash"]+=money
            win.append(f"• {b['name']} `+{money:,}$ vào ví`")
        else:
...
async def taocode(ctx,n:int=None,uses:int=None):
    ...
    if not n or not uses:return await ctx.send("❌ `!taocode 10000 5`")
    c=code();C[c]={"money":n,"uses":uses,"used":set()}
    await ctx.author.send(f"🔐 Code: `{c}` — 💰 `{n:,}$` — {uses} lượt")
    await ctx.send("✅ Đã gửi code vào DM.")

@bot.command()
async def thuongcode(ctx,n:int=None,uses:int=None):
    if not adm(ctx):return await ctx.send("⛔ Chỉ Admin!")
    if not n or not uses:return await ctx.send("❌ `!thuongcode 10000 5`")
    c=code();C[c]={"money":n,"uses":uses,"used":set()}
    await ctx.send(f"🎁 **CODE:** `{c}` — 💰 `{n:,}$` — `{uses}` lượt")

@bot.command()
async def nhapcode(ctx,c=None):
    c=(c or "").upper()
    if c not in C:return await ctx.send("❌ Code không tồn tại!")
    ...
    await ctx.send(f"🎁 Nhận **+{x['money']:,}$ vào ví**!")
