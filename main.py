...
async def dd(ctx):
    i=ctx.author.id
    now=time.time()
    if i in DD and now-DD[i]<43200:
        left=int(43200-(now-DD[i]))
        h=left//3600
        m=(left%3600)//60
        return await ctx.send(f"⚠️ Bạn đã điểm danh! Còn **{h} giờ {m} phút**.")
    DD[i]=now
    u=user(i,ctx.author.name)
    u["cash"]+=2593
    await ctx.send(embed=E("🎁 ĐIỂM DANH",
    f"💰 Nhận **+2,593$**\n💵 Ví: `{u['cash']:,}$`\n"
    f"⏰ Điểm danh lại sau **12 giờ**.",GREEN))
...
async def tx(ctx,choice=None,bet:int=None):
    ...
    if not TX["on"]:
        ...
        "Gõ `!tx <tai/xiu> <tiền>` **(Tối đa 10,000,000$/ván)**\n\n"
        "⏱️ **Thời gian: 30 giây**\n\n"
    ...
    e.description=(
        "Gõ `!tx <tai/xiu> <tiền>` **(Tối đa 10,000,000$/ván)**\n\n"
        "⏱️ **Thời gian: 30 giây**\n\n"
        f"💰 Tổng Tài: `{TX['tai']:,}$` | Tổng Xỉu: `{TX['xiu']:,}$`"
    )
...
async def txround():
    ...
    await m.edit(embed=E("🎲 ĐANG XÓC BÁT",
    "🥣 **XÓC... XÓC... XÓC...**\n\n`[ . ] [ . ] [ . ]`",ORANGE))
