...
async def vi(ctx, m: discord.Member = None):
    ...
    await ctx.send(embed=E(
        f"💳 THẺ TÀI KHOẢN: {m.display_name}",
        f"👑 **Hạng:** Người chơi Thường\n\n"
        f"💵 **Tiền mặt**\n`{fm(u['cash'])}`\n\n"
        f"🏦 **Ngân hàng**\n`{fm(u['bank'])}`\n\n"
        f"💰 **Tổng tài sản**\n**{fm(u['cash'] + u['bank'])}**\n\n"
        f"💸 **Khoản vay:** `{fm(loan)}`\n"
...
async def bxh(ctx):
    ...
    for i, (uid, u) in enumerate(arr, 1):
        ...
        text += f"**{i}.** {name} — 💰 `{fm(u['cash'] + u['bank'])}`\n"
...
async def trano(ctx, amount: int = None):
    ...
    if amount != debt:
        return await ctx.send(f"❌ Phải trả đúng **{fm(debt)}**.")
...
async def quay(ctx, amount: int = None):
    ...
    await msg.edit(embed=E("🎰 MÁY SLOT NỔ HŨ", text, color))
...
async def bc(ctx, choice: str = None, amount: int = None):
    ...
    board = "  ".join(f"【 {icons[x]} 】" for x in result)
    ...
    await msg.edit(embed=E("🦀 BẦU CUA TÔM CÁ", text, color))
...
async def xd(ctx, choice: str = None, amount: int = None):
    ...
    result_name = "CHẴN" if result == "chan" else "LẺ"
...
async def tx(ctx, choice: str = None, amount: int = None):
    ...
    if amount < 100 or amount > 10000000:
        return await ctx.send("❌ Cược không hợp lệ.")
    ...
    if choice == result:
        ...
        text = (
            f"🎲 **Xúc xắc:** `{dice[0]} - {dice[1]} - {dice[2]}`\n"
    ...
    await msg.edit(embed=E("🎲 KẾT QUẢ TÀI XỈU", text, color))
...
async def nhapcode(ctx, code: str = None):
    if not code:
        return await ctx.send("❌ Nhập code.")
...
async def settien(ctx, m: discord.Member = None, amount: int = None):
    ...
    await ctx.send(
        f"🛡️ Đã set tiền {m.mention} → **{fm(amount)}**."
    )
...
async def reset(ctx, what: str = None, m: discord.Member = None):
    ...
    await ctx.send(
        f"♻️ {m.mention} đã reset về **{fm(START)}**."
    )
...
async def unkhoamom(ctx, m: discord.Member = None):
    ...
    if not m:
        return await ctx.send("❌ `!unkhoamom @user`")

    U(m)["muted"] = False
    await ctx.send(
        f"🔊 {m.mention} đã được **mở khóa mõm**.\n"
        "🟢 Có thể chơi lại."
    )
