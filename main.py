...
users = {}
codes_db = {"BET88": 50000, "VIP2026": 100000} # Kho code mẫu để test lệnh !nhapcode
...
@bot.event
async def on_ready():
    print(f"✅ BOT FULL CHỨC NĂNG ĐÃ SẴN SÀNG: {bot.user}")

# --- MENU TRỢ GIÚP ĐẦY ĐỦ ---
...
async def bxh_cmd(ctx):
    ...
    desc = "🏆 **TOP Ù TÀI PHÚ HỘ HÀNG ĐẦU** 🏆\n"
...
async def rl_cmd(ctx, color_choice: str = None, bet: int = None):
    ...
    if rand_val <= 10: result_color = "xanh"
    elif rand_val <= 35: result_color = "den"
    else: result_color = "do"
...
async def quay_slot_cmd(ctx, bet: int = None):
...
async def xocdia_cmd(ctx, choice: str = None, bet: int = None):
    ...
    kq = "chan" if red % 2 == 0 else "le"
    if choice.lower() == kq:
        u["cash"] += bet
        await ctx.send(f"🪙 Xóc đĩa ra {red} Đỏ ➔ **{kq.upper()}**\n🎉 **Thắng!** `+{bet:,} $`")
    else:
        u["cash"] -= bet
        await ctx.send(f"🪙 Xóc đĩa ra {red} Đỏ ➔ **{kq.upper()}**\n💸 **Thua!** `-{bet:,} $`")
...
async def baucua_cmd(ctx, choice: str = None, bet: int = None):
    ...
    if matches > 0:
        ...
        u["cash"] += win
        await ctx.send(f"🎲 Mở bát: {animals[d1]} {animals[d2]} {animals[d3]}\n✨ **Trúng {matches} con!** Nhận `+{win:,} $`")
    else:
        u["cash"] -= bet
        await ctx.send(f"🎲 Mở bát: {animals[d1]} {animals[d2]} {animals[d3]}\n😢 **Tróc vẩy!** `-{bet:,} $`")

# --- NHÓM LỆNH ĐỐI KHÁNG (PVP) KHUNG MẪU ---
...
async def danhbai_pvp(ctx, member: discord.Member = None, bet: int = None):
    ...
    await ctx.send(f"⚔️ **{ctx.author.name}** đã thách đấu đánh bài với **{member.name}** mức cược `{bet:,} $`! *(Tính năng đối kháng PVP đang sẵn sàng tích hợp)*")
